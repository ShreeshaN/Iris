import numpy as np
from src.iris import graph_plotter
...
class Network:
    ...
    def update_biases_weights(self,new_biases,new_weights):
...
if (__name__=='__main__'):
    filepath = 'Iris.csv'
    train_data, test_data = data_loader.loadNclean_data(filepath)
    ...
    for x in range(epochs):
        for data in train_data:
            ...
            net.update_biases_weights(new_biases,new_weights)

            # Each cost sample has 3 values coming from 3 output neurons , so finding the mean of them
            net.cost_per_sample_list.append(sum(cost_per_sample) / len(cost_per_sample))

        # Calculate the quadratic cost for each epoch
        cost_per_epoch = net.quadratic_cost_func(None, None, (sum(net.cost_per_sample_list) / float(len(net.cost_per_sample_list))), True)
